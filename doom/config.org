...
(setq user-full-name "Stuart Yi-Thomas"
...
(after! org
        ...
        (plist-put org-format-latex-options :html-scale 3.0)
...
(defun org-babel-goto-src-block-head-or-previous ()
  (interactive)
  (let ((head (org-babel-where-is-src-block-head)))
    (if (or (not head) (= (point) head))
        (org-babel-previous-src-block)
        (org-babel-goto-src-block-head))))

(map! (:map org-mode-map :desc "Previous src block" :n "K" #'org-babel-goto-src-block-head-or-previous))
(map! (:map org-mode-map :desc "Next src block"     :n "J" #'org-babel-next-src-block))
(map! (:after evil :map evil-motion-state-map "K" nil))
(map! (:after evil :map evil-normal-state-map "K" nil))
(map! (:after evil :map evil-visual-state-map "K" nil))
(map! (:after evil :map evil-normal-state-map "J" nil))
...
#+begin_src emacs-lisp
(map! :leader :desc "Open Mu4e" :g "o m" #'(lambda ()
                                            (interactive)
                                            (mu4e t)
                                            (mu4e~headers-jump-to-maildir "/umd/INBOX")
                                            (mu4e-view-refresh)
                                            ))
#+end_src
  ...
  (clear-minibuffer-message)
   (let* (
    ...
    (new-pdf-path (concat project project-references-directory "/" key ".pdf")))
     ...
     (rename-file pdf-path new-pdf-path)))


;;; DOESNT WORK
(set-file-template! "bib.org" :when (lambda (fname)
                                        (insert (format """
:PROPERTIES:
:header-args: :tangle bib.bib :tangle bib.bib :results silent
:END:
,#+title: %s Bibliography
,#+STARTUP: fold
,#+auto_tangle: t
---
""")))
...
(defun my/open-doi (doi &optional fname)
  (message "my/open-doi: Opening link: %s" uri)
  (let ((filename (if fname fname (make-temp-file "stumacs" nil ".pdf"))))
    ;;(shell-command-to-string (concat "doi2pdf " doi " " fname))))
    (shell-command-to-string (format "doi2pdf \"%s\" \"%s\"" doi filename))
    (find-file filename)
    ))
...
(defun open-link (uri &optional fname default-open-function)
 ...
 URI: the uri"
  ...
  (unless default-open-function (setq default-open-function #'browse-url-default-browser))
  (cond
        ((string-match arxiv-code-regex uri) (my/open-doi (match-string 1 uri) fname)) ;; new style, DOI
        ((string-match arxiv-old-code-regex uri) (my/open-doi (match-string 1 uri) fname)) ;; old style, DOI
        ((string-match arxiv-link-regex uri) (my/open-doi (match-string 2 uri) fname)) ;; new style, url
        ((string-match arxiv-old-link-regex uri) (my/open-doi (match-string 2 uri) fname)) ;; old style, url
        ((string-match doi-regex uri) (my/open-doi (match-string 0 uri) fname))
        ( t (funcall default-open-function uri))))
...
(defun doi-of-pdf (filename)
    (let* ((res (shell-command-to-string (concat "pdf2doi " filename))))
        (when (string-equal (nth 0 (string-split res " " t)) "Error:") (error (concat "pdf2doi returned error:\n" res)))
...
(setq julia-vterm-repl-program "~/.juliaup/bin/julia")
(defun my/julia-repl ()
...
(defun org-babel-execute:bibtex (BODY PARAMS)
  (let*
   ((key (progn (string-match "{\\(.+\\)," BODY) (match-string 1 BODY)))
    (root (projectile-acquire-root))
    (fname (format "%sreferences/%s.pdf" root key)))
   (if (file-exists-p fname)
       (find-file fname)
       (open-link BODY fname)))) ;; find the first link and open it
...
(after! elfeed
  ...
  (setq elfeed-feeds '(("http://10.8.0.2:8000/feed.xml" arxiv)))
  (advice-add
...
(use-package! calibredb
  ...
  (setq calibredb-library-alist '(("~/Textbooks")))
