...
-- myExit = do {
--   emacsstatus <- io $ readProcess "emacsclient" ["-e", "(save-buffers-kill-emacs)"] "";
--   }

myExit = do {
  x <- liftIO $ readProcess "emacsclient" ["-e", "(save-buffers-kill-emacs)"] [];
  io $ putStrLn $ "<" ++ x ++ ">";
  if (null x) then io (exitWith ExitSuccess) else return ()
}

  -- spawn $ "notify-send \"" ++ emacsstatus ++ "\" -t 1000"
  -- if (emacsstatus == "")
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm .|. shiftMask, xK_t     ), spawn "flatpak run com.toggl.TogglDesktop") -- launch toggl
